...
RSpec.describe Mutations::UpdateUserMutation do
  ...
  let(:query) do
    %(mutation {
      updateUser(firstName: "Modified", lastName: "Updated") {
        user {
          id
          fullName
        }
  ...
  let(:query_update_last_name) do
    %(mutation {
      updateUser(firstName: null, lastName: "Updated") {
        user {
          id
          fullName
        }
  ...
  let(:query_update_first_name) do
    %(mutation {
      updateUser(firstName: "Modified", lastName: null) {
        user {
          id
          fullName
        }
  ...
  let(:query_invalid_params) do
    %(mutation {
      updateUser(firstName: "#{'a' * 129}", lastName: "#{'b' * 129}") {
        user {
          id
          fullName
        }
  ...
  it 'modifies the user object on success' do
    expect do
      IdpSchema.execute(query, context: { current_user: user }).as_json
      user.reload
  ...
  it 'returns the modified user on success when only the first name is provided' do
    result = IdpSchema.execute(query_update_first_name, context: { current_user: user }).as_json
  ...
  it 'modifies the user first name on success when only the first name is provided' do
    expect do
      IdpSchema.execute(query_update_first_name, context: { current_user: user }).as_json
      user.reload
  ...
  it 'returns the modified user on success when only the last name is provided' do
    result = IdpSchema.execute(query_update_last_name, context: { current_user: user }).as_json
  ...
  it 'modifies the user last name on success when only the last name is provided' do
    expect do
      IdpSchema.execute(query_update_last_name, context: { current_user: user }).as_json
      user.reload
  ...
  it 'returns an array of errors for each error when update fails' do
    result = IdpSchema.execute(query_invalid_params, context: { current_user: user }).as_json
